...
var Editor = function(path) {
  this._path   = path;
  this._editor = process.env.VISUAL || process.env.EDITOR;
};

Editor.DEFAULT_EDITOR = 'vim';

Editor.editTempfile = function(content) {
  var path = '/tmp/vault-notes-' + crypto.randomBytes(16).toString('hex');

  return this.edit(path, content).then(function(result) {
    fs.unlinkSync(path);
    return result;
  });
};
