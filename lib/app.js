...
(function() {
  ...
  var fetchSettings = function() {
    ...
    store.serviceSettings(service.value, true, function(error, settings) {
      if (error) return; // TODO flag up this important error
...
var store = (function() {
  ...
  if (token) {
    params.state = userId;
    bookmark = url.protocol + '//' + url.host + '/#' + querystring(params);
    store = new RemoteStore(userId, {token: token});
  ...
  var connect = function(userId) {
    store = new RemoteStore(userId);
    if (local) local.userId = userId;
    feedback.innerHTML = 'Connecting&hellip;';
    store.connect(function(error) { feedback.innerHTML = error.message });
  };
