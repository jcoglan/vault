...
Vault.prototype.generate = function(service) {
  if (this._required.length > this._length)
    return Promise.reject(Error('Length too small to fit all required characters'));

  if (this._allowed.length === 0)
    return Promise.reject(Error('No characters available to create a password'));

  var self = this;

  return Vault.createHash(this._phrase, service + Vault.UUID, 2 * this.entropy()).then(function(hash) {
    var required = self._required.slice(),
        stream   = new Sequin(hash, 8),
        result   = '',
        index, charset, previous, i, same;

    while (result.length < self._length) {
      ...
      while (same && i--)
        same = same && result.charAt(result.length + i - self._repeat) === previous;
      if (same)
        charset = self.subtract([previous], charset.slice());
