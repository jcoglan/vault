...
Vault.createHash = function(key, message, entropy, iteration) {
  var CJS   = (typeof CryptoJS !== 'undefined') ? CryptoJS : require('./crypto-js-3.1.2'),
      bytes = (entropy || 256) / 8;

  return CJS.PBKDF2(key, message, {keySize: Math.ceil(bytes / 4), iterations: iteration}).toString();
};
...
Vault.prototype.generate = function(service) {
  ...
  var required = this._required.slice(),
      stream   = new Vault.Stream(this._phrase, service, this.entropy(), this._iteration),
      result   = '',
...
Vault.Stream = function(phrase, service, entropy, iteration) {
  this._phrase  = phrase;
  this._service = service;

  var hash = Vault.createHash(phrase, service + Vault.UUID, 2 * entropy, iteration),
