...
Vault.Stream = function(phrase, service, entropy) {
  this._hash   = Vault.createHash(phrase, service + Vault.UUID, entropy);
  this._bits   = Vault.map(this._hash.split(''), Vault.toBits).join('');
  this._offset = 0;
};

Vault.Stream.prototype.generate = function(base) {
  if (base === 1) return 0;
  
  var size  = Math.ceil(Math.log(base) / Math.log(2)),
      chunk = this._bits.substr(this._offset, size);
  ...
  value = parseInt(this._bits.substr(this._offset, size), 2);
  this._offset += size;
  return value;
};
